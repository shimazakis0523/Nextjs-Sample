#!/usr/bin/env bash
# ユースケース記述<N>_*.md / 画面定義書<N>_*.md を変更したPRが、対応する
# E2E仕様書<N>_*.md(常に)と画面遷移図.md(遷移先を記述する行を変更した場合)を
# 同じPRで追従させているかを検査する。
#
# constitution.md Development Workflow:
#   - 画面のユースケース記述・画面定義書の変更は update-e2e-test-spec スキルで
#     対応するE2E仕様書<N>_*.mdを追従させること
...
while IFS= read -r design_file; do
  ...
  # ファイル名は「(ユースケース記述|画面定義書)<N>_<画面名>.md」の形式。Nを取り出す。
  screen_num="$(echo "$filename" | sed -E 's#^(ユースケース記述|画面定義書)([0-9]+)_.*#\2#')"
  screen_flow_file="$business_dir/画面遷移図.md"

  e2e_test_spec_changed="$(echo "$CHANGED_FILES" \
    | grep -E "^$(printf '%s' "$business_dir" | sed 's/[.[\*^$/]/\\&/g')/E2E仕様書${screen_num}_" || true)"

  if [ -z "$e2e_test_spec_changed" ]; then
    echo "NG: $design_file を変更していますが $business_dir/E2E仕様書${screen_num}_*.md が同じ変更に含まれていません。" >&2
...
done < <(echo "$CHANGED_FILES" | grep -E '^doc/フロントエンド設計書/[^/]+/(ユースケース記述|画面定義書)[0-9]+_[^/]+\.md$' || true)
