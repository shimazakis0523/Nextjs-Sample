...
1. Run `.specify/scripts/bash/check-prerequisites.sh --json --require-tasks --include-tasks` from repo root and parse FEATURE_DIR and AVAILABLE_DOCS list. **Path note (ADR-0013, ADR-0014)**: this script still resolves paths under the legacy `specs/<feature>/` layout, which no longer exists — treat its FEATURE_DIR only as a way to identify which business this run is for (the `<feature-dir-name>` segment), never as a literal path to read from. Resolve the real locations instead:
   - If `.specify/feature.json` has `tracking_directory`/`business_directory` fields (set by a `/speckit-specify` run under the current architecture), use them directly as `TRACKING_DIR`/`BUSINESS_DIR`.
   - Otherwise (an older feature predating this field), map `<feature-dir-name>` (e.g. `001-todo-dashboard`) to its directories by matching existing `doc/フロントエンド設計書/<業務>/` and `tracking/<業務>/` directories, asking the user if no clear match exists.
   Set `TASKS_FILE` to `TRACKING_DIR/tasks.md` — this is what step 3 actually reads, not a path under FEATURE_DIR. For single quotes in args like "I'm Groot", use escape syntax: e.g 'I'\''m Groot' (or double-quote if possible: "I'm Groot").

2. **Check checklists status** (if `TRACKING_DIR/screens/*/checklists/` exists):
   ...
   - Scan all checklist files under every `TRACKING_DIR/screens/<screen-id>/checklists/` directory
...
3. Load and analyze the implementation context:
   - **REQUIRED**: Read `TASKS_FILE` (`TRACKING_DIR/tasks.md`) for the complete task list and execution plan
   - **REQUIRED**: Read `BUSINESS_DIR/詳細設計書.md` for tech stack, architecture, and file structure
   - **REQUIRED**: Read every `BUSINESS_DIR/ユースケース記述_*.md` and `画面定義書_*.md` this task set
     touches, for the screen-level behavior a task must satisfy (詳細設計書 only covers
     component wiring, not per-screen requirements)
   - **IF EXISTS**: Read `.specify/memory/constitution.md` for governance constraints
   - Feature directories under this project's `doc/フロントエンド設計書/` never contain
     `data-model.md`, `contracts/`, `research.md`, or `quickstart.md` (constitution.md
     Principle VI forbids them) — do not look for them
...
4. **Project Setup Verification**:
   ...
   **Detection & Creation Logic**:
   ...
   - Check if Dockerfile* exists or Docker in 詳細設計書.md → create/verify .dockerignore
   ...
   **Common Patterns by Technology** (from 詳細設計書.md tech stack):
