...
<!--
  ...
  - props/callbackのラベルは名前だけで済ませず、必ずフィールドレベルの型まで書く
    (例: "props: todos: Todo[]"、"callback: onSaved(todo: Todo) => void"。
    "props: todos"のような名前のみの記載は禁止)。型が`doc/API仕様書/common/schemas/**`
    に定義済みのエンティティ(例: Todo)であれば、そのエンティティ名をそのまま使い、
    フィールドを再列挙しない。共有定義が無いローカル専用の型のみ、その場でフィールドを
    書き下す。型を省略すると、実装者は既存コード(または想像)から型を推測するしかなく
    なり、設計書としての意味を失う。
